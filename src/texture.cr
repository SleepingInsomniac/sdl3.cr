module Sdl3
  class Texture < SdlObject(LibSdl3::Texture*)
    ...
    def unsafe_lock(rect : Rect? = nil) : Tuple(Pointer(Void), Int32)
    ...
    def lock_to_surface(rect : Rect?, & : Surface ->)
      result = LibSdl3.lock_texture_to_surface(self, rect, out surface_ptr)
      Sdl3.raise_error unless result

      begin
        yield Surface.new(surface_ptr, owned: false)
      ensure
        LibSdl3.unlock_texture(self)
      end
    end
